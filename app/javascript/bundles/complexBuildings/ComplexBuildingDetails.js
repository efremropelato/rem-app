...
export default class AssetDetails extends React.Component {
  ...
  componentDidMount() {
    ...
    const a = fetch(`/api/v1/complex_buildings/${id}`).then((response) => response.json())
    const i = fetch(`/api/v1/assets/ComplexBuilding/${id}/images`).then((response) => response.json())
  ...
  buyAsset(){
    const { match: { params: { id } } } = this.props;
    fetch(`/api/v1/assets/ComplexBuilding/${id}/buy`, {
      method: 'post',
      body: null,
    ...
    return (
      <Container>
        <Row>
          <Col>
            <Card>
              <CardHeader>Complex Buildings {asset.id}</CardHeader>
              <CardBody>
                <CardText>
                  <div>
                    <label> Owner </label>
